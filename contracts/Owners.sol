...
/* Constants */
    ...
    int constant HEAD = 0; // Lists are circular with static head.
    bool constant PREV = false;
    bool constant NEXT = true;
    int constant MAXNUM = int(-1); // 2**256 - 1
    ...
    function ordered(int n) {
        int m = list.seek(HEAD, n, NEXT); // Find first number larger than n
        list.insert(m, n, PREV); // insert n before m
    }
    
    // As a mapping key store or lookup
    mapping (int => address) map;
    
    function insert(int id, address addr) {
        map[id] = addr;
